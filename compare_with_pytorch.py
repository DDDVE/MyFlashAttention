...
output_dir = f"flash_atten_1_2_compare_{timestamp}"
    ...
    for H in H_values:
        for d in d_values:
            ...
            for N in N_values:
                ...
                # 运行flash attention-1
                time_fa1 = flash_attention.flash_attention_best(Q, K, V, O, B, H, N, d)
                # diff = QKV-O
                # print("diff:", diff)
                assert torch.allclose(QKV,O, atol=1e-2), "flash attention-1 result wrong"

                # 运行flash attention-2
                O[:B, :H, :N, :d].zero_()
                time_fa2 = flash_attention2.flash_attention2(Q, K, V, O, B, H, N, d)
                assert torch.allclose(QKV,O, atol=1e-2), "flash attention-2 result wrong"
